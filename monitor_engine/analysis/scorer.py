...
def _dedup_entities(raw: list[_EntityResult]) -> list[Entity]:
    """Normalize LLM-extracted entities: trim names, lowercase types, drop blanks,
    ...
    for e in raw:
        ...
        if key in seen:
            continue
        seen.add(key)
        out.append(Entity(name=name, type=(e.type or "other").strip().lower() or "other"))
    return out
...
class _RunCost:
    ...
    def print_summary(self, *, deep_item_count: int = 0) -> None:
        for phase in (PHASE_CLASSIFICATION, PHASE_RESEARCH, PHASE_DEEP_ANALYSIS, PHASE_EDITORIAL):
            entry = self._phases.get(phase)
            if not entry:
                continue
...
class Scorer:
    ...
    def _assemble_item(self, llm: _ItemResult, raw: RawItem) -> AnalyzedItem | None:
        ...
        entities = _dedup_entities(llm.entities)
    ...
    def _deep_analysis_batch(
        ...
        *research_notes* (item_id → notes) from the agentic pass are folded into
        ...
        result = self._call_json(
            ...
            build_deep_analysis_user_prompt(batch, research_notes), schema,
            self._deep_max_tokens(len(batch)),
    ...
    def _run_research(
        ...
        """Agentic research pass. Returns {item_id: notes} for items whose tier is
        in ``deep_analysis.agentic_tiers``. No-op (empty dict) when agentic is off.
        Best-effort: any per-item failure leaves that item without notes."""
        da = self._config.deep_analysis
        if da is None or not da.agentic:
            return {}
        tiers = set(da.agentic_tiers)
        targets = [a for a in analyzed if a.tier in tiers]
        if not targets:
            return {}

        from monitor_engine.analysis.research import ResearchAgent
        ...
        agent = ResearchAgent(
            ...
            on_usage=lambda usage: cost.add(PHASE_RESEARCH, CHEAP_MODEL, usage),
        )

        def _one(item: AnalyzedItem) -> tuple[str, str]:
            out = agent.research(item, raw_by_id.get(item.item_id))
            if out.is_empty:
                return item.item_id, ""
            notes = out.notes
            if out.sources:
                notes += "\nSources consulted: " + ", ".join(out.sources)
            return item.item_id, notes

        notes: dict[str, str] = {}
        # Reuse the wave/cap machinery so research respects the output-token cap.
        for item, (iid, text) in zip(
            targets,
            self._run_capped(targets, _one, cost, caps),
        ):
            if text:
                notes[iid] = text
        logger.info("Agentic research produced notes for %d/%d item(s)", len(notes), len(targets))
        return notes

    def _research_session(self) -> Any:
    ...
    def _attach_deep_analysis(
        ...
        # Optional agentic research pass: top-tier items get a tool-using research
        # loop whose findings are folded into the deep-analysis prompt.
        research_notes = self._run_research(analyzed, raw_items, cost, caps)
        ...
        batch_results = self._run_capped(
            batches,
            lambda b: self._deep_analysis_batch(b, system_prompt, schema, cost, research_notes),
            cost, caps,
        )
