...
def run_pipeline(
    ...
    *,
    ...
    # items that share entities. Both run on the deduped set so what's published,
    # archived, and graphed is identical. Enrichment is fail-soft per entity.
    deduped = dedup_items(analyzed)

    entity_index: list = []
    if deduped:
        if config.enrichers:
            from monitor_engine.enrichment import enrich_items
            result = enrich_items(config, deduped)
            deduped = result.items
            logger.info(
                "Enrichment: %d fact(s) from %d enricher(s), %d lookup error(s)",
                result.total_facts, len(result.stats), result.total_errors,
            )
        from monitor_engine.enrichment import build_entity_graph
        deduped, entity_index = build_entity_graph(deduped)
        if entity_index:
            logger.info("Entity graph: %d connective entit(y/ies)", len(entity_index))
