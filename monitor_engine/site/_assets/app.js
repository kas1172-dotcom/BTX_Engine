...
function boot() {
  ...
  applyBranding(cfg);
  buildAccountMapLink(cfg);
  buildIntegrationsBadge(cfg);
  buildEditionNav(cfg);
...
function fullCard(item) {
  ...
  div.setAttribute('id', 'item-' + item.item_id);   // anchor target for related links
  ...
  const enrich = enrichmentBlock(item);
  if (enrich) div.appendChild(enrich);

  const related = relatedBlock(item);
  if (related) div.appendChild(related);
...
function compactRow(item) {
  ...
  details.setAttribute('id', 'item-' + item.item_id);   // anchor target for related links
  ...
  if (ea) {
    ...
    const enrich = enrichmentBlock(item);
    if (enrich) detail.appendChild(enrich);

    const related = relatedBlock(item);
    if (related) detail.appendChild(related);
...
/* ═══════════════════════════════════════════════════════════════════
   Enrichment — cross-API facts attached to an item
   ═══════════════════════════════════════════════════════════════════ */
// Map enricher_id → display label from site_config, for grouping/headers.
function enricherLabel(id) {
  const cfg = state.data && state.data.site_config;
  const list = (cfg && cfg.enrichers) || [];
  const hit = list.find(e => e.id === id);
  return (hit && hit.label) || id;
}

function factValueText(f) {
  if (f.kind === 'money' && f.number != null) return fmtMoney(f.number, 'USD');
  if (f.kind === 'number' && f.number != null) return fmtPop(f.number);
  return f.value;
}

// "Live data" block: structured facts pulled from other APIs for this item's
// entities. Grouped by the entity they were looked up for.
function enrichmentBlock(item) {
  const enr = item.enrichment;
  const facts = (enr && enr.facts) || [];
  if (!facts.length) return null;

  const wrap = el('div', 'enrichment-block');
  const label = el('span', 'analysis-label');
  label.textContent = 'Live data';
  wrap.appendChild(label);

  // Group facts by entity so multiple lookups read cleanly.
  ...
  byEntity.forEach((group, entity) => {
    const grp = el('div', 'enrich-group');
    const head = el('div', 'enrich-entity');
    head.textContent = entity;
    const badge = el('span', 'enrich-source');
    badge.textContent = enricherLabel(group[0].enricher_id);
    head.appendChild(badge);
    grp.appendChild(head);

    const facts_wrap = el('div', 'enrich-facts');
    group.forEach(f => {
      const chip = el('div', 'enrich-fact' + (f.kind === 'money' ? ' money' : ''));
      const k = el('span', 'enrich-label');
      k.textContent = f.label;
      chip.appendChild(k);
      const text = factValueText(f);
      if (f.url) {
        chip.appendChild(safeLink(f.url, text, 'enrich-value'));
      } else {
        const v = el('span', 'enrich-value');
        v.textContent = text;
        chip.appendChild(v);
      }
      facts_wrap.appendChild(chip);
    });
    grp.appendChild(facts_wrap);
...
/* ═══════════════════════════════════════════════════════════════════
   Related — entity-graph edges to other items in this brief
   ═══════════════════════════════════════════════════════════════════ */
function relatedBlock(item) {
  const refs = (item && item.related) || [];
  if (!refs.length) return null;
  const wrap = el('div', 'related-block');
  const label = el('span', 'analysis-label');
  label.textContent = 'Connected stories';
  wrap.appendChild(label);
  refs.forEach(r => {
    const row = el('div', 'related-row');
    // Native in-page anchor: cards carry id="item-<item_id>".
    const a = el('a', 'related-link');
    a.href = '#item-' + r.item_id;
    a.textContent = cleanTitle(r.title);
    a.title = r.title;
    row.appendChild(a);
    if (r.via && r.via.length) {
      const via = el('span', 'related-via');
      via.textContent = 'via ' + r.via.join(', ');
      row.appendChild(via);
    }
...
/* ═══════════════════════════════════════════════════════════════════
   Entity explorer — the run-level graph as a browsable index
   ═══════════════════════════════════════════════════════════════════ */
function buildEntityExplorer() {
  const idx = (state.data && state.data.entity_index) || [];
  if (!idx.length) return;

  const header = document.getElementById('site-header');
  const btn = el('button', 'entity-map-btn');
  btn.type = 'button';
  btn.textContent = '🕸 Entity map (' + idx.length + ')';
  btn.setAttribute('aria-expanded', 'false');
  header.appendChild(btn);

  const overlay = document.getElementById('main-layout');
  const panel = el('div', 'entity-panel');
  panel.hidden = true;
  const title = el('p', 'entity-panel-title');
  title.textContent = 'Entities connecting this brief — click to filter';
  panel.appendChild(title);

  const grid = el('div', 'entity-grid');
  idx.forEach(e => {
    const node = el('button', 'entity-node');
    node.type = 'button';
    const nm = el('span', 'entity-name');
    nm.textContent = e.name;
    const ct = el('span', 'entity-count');
    ct.textContent = e.item_ids.length;
    node.appendChild(nm);
    node.appendChild(ct);
    node.title = e.type + ' · appears in ' + e.item_ids.length + ' items';
    node.addEventListener('click', () => {
      const input = document.getElementById('search-input');
      if (input) { input.value = e.name; state.searchQuery = e.name.toLowerCase(); }
      panel.hidden = true;
      btn.setAttribute('aria-expanded', 'false');
      render();
    });
...
// "Account Map" link in the header — shown when the deployment ships a map page.
function buildAccountMapLink(cfg) {
  const url = cfg && cfg.account_map_url;
  if (!url) return;
  const header = document.getElementById('site-header');
  const a = el('a', 'account-map-link');
  a.href = url;
  a.textContent = '🗺 Account Map';
  header.appendChild(a);
}

// "Live integrations" badge in the header — names the active cross-API sources.
function buildIntegrationsBadge(cfg) {
  const enr = (cfg && cfg.enrichers) || [];
  if (!enr.length) return;
  const header = document.getElementById('site-header');
  const badge = el('div', 'integrations-badge');
  badge.textContent = '⚡ Live integrations: ' + enr.map(e => e.label).join(' · ');
