  ...
  return new Promise(resolve => {
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        // Signal Mesh probe: cross-API enrichment, entity-graph edges, explorer.
        const header = doc.getElementById('site-header');
        const layout = doc.getElementById('main-layout');
        const sumOver = (cls) => containers.tier1.countByClass(cls)
          + containers.tier2.countByClass(cls) + containers.tier3.countByClass(cls);
        const badge = header.findByClass('integrations-badge');
        result.mesh = {
          enrichmentBlocks: sumOver('enrichment-block'),
          enrichFacts: sumOver('enrich-fact'),
          relatedBlocks: sumOver('related-block'),
          relatedLinks: sumOver('related-link'),
          entityNodes: layout.countByClass('entity-node'),
          entityMapBtn: header.findByClass('entity-map-btn') ? true : false,
          integrationsBadge: badge ? badge.textContent : null,
          accountMapLink: (header.findByClass('account-map-link') || {}).href || null,
        };
