...
# ─── Entity extraction & agentic research wiring ───────────────────────────

class TestEntityExtraction:
    def _payload_with_entities(self, item_id: str, entities: list[dict]) -> dict:
        p = _batch_payload(item_id)
        p["results"][0]["entities"] = entities
        return p

    def test_classification_extracts_entities(self, config):
        raw = _raw()
        payload = self._payload_with_entities(raw.id, [
            {"name": "Lockheed Martin", "type": "organization"},
            {"name": "F-35", "type": "program"},
        ])
        mock_client = MagicMock()
        mock_client.messages.create.return_value = _mock_response(payload)
        scorer = Scorer(config, client=mock_client)
        items, _ed, _cost = scorer.analyze([raw])
        names = {e.name for e in items[0].entities}
        assert names == {"Lockheed Martin", "F-35"}
        assert {e.type for e in items[0].entities} == {"organization", "program"}

    def test_entities_deduped_and_normalized(self):
        from monitor_engine.analysis.scorer import _dedup_entities, _EntityResult
        out = _dedup_entities([
            _EntityResult(name="RTX", type="Organization"),
            _EntityResult(name="rtx", type="organization"),   # dup (case-insensitive)
            _EntityResult(name="  ", type="program"),          # blank → dropped
            _EntityResult(name="F-35", type=""),               # empty type → "other"
        ])
        assert [(e.name, e.type) for e in out] == [("RTX", "organization"), ("F-35", "other")]

    def test_missing_entities_defaults_to_empty_list(self, config):
        raw = _raw()
        mock_client = MagicMock()
        mock_client.messages.create.return_value = _mock_response(_batch_payload(raw.id))
        scorer = Scorer(config, client=mock_client)
...
class TestResearchNotesPrompt:
    def test_deep_prompt_includes_research_notes(self):
        from monitor_engine.analysis.prompts import build_deep_analysis_user_prompt
        raw = _raw(item_id="x1")
        prompt = build_deep_analysis_user_prompt([raw], {"x1": "Found a $5M award via USAspending."})
        assert "RESEARCH NOTES" in prompt
        assert "Found a $5M award" in prompt

    def test_deep_prompt_without_notes_is_unchanged(self):
        from monitor_engine.analysis.prompts import build_deep_analysis_user_prompt
        raw = _raw(item_id="x1")
        assert "RESEARCH NOTES" not in build_deep_analysis_user_prompt([raw])
        assert "RESEARCH NOTES" not in build_deep_analysis_user_prompt([raw], {"other": "x"})
