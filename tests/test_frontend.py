...
# ─── Signal Mesh: enrichment, entity graph, explorer ───────────────────────

def _with_mesh(fixture_data):
    """Inject enrichment facts, graph edges, an entity index, and active
    enrichers into a deep copy of the fixture for the mesh render probes."""
    data = json.loads(json.dumps(fixture_data))
    items = data["items"]
    ids = [it["item_id"] for it in items]
    items[0]["enrichment"] = {
        "facts": [
            {"enricher_id": "awards", "entity": "RTX", "label": "Top recent award",
             "value": "$5,000,000", "kind": "money", "number": 5000000.0, "url": None},
            {"enricher_id": "awards", "entity": "RTX", "label": "Awarding agency",
             "value": "DoD", "kind": "text", "number": None, "url": None},
        ],
        "queried_entities": ["RTX"],
    }
    items[0]["related"] = [
        {"item_id": ids[1], "title": items[1]["title"], "url": items[1]["url"], "via": ["RTX"]},
    ]
    data["entity_index"] = [
        {"name": "RTX", "type": "organization", "item_ids": ids[:2]},
        {"name": "F-35", "type": "program", "item_ids": ids[:3]},
    ]
    data.setdefault("site_config", {})["enrichers"] = [
        {"id": "awards", "label": "Federal awards (USAspending)"},
    ]
    return data


@pytest.fixture(scope="module")
def mesh_rendered(fixture_data, tmp_path_factory):
    p = tmp_path_factory.mktemp("mesh") / "mesh.json"
    p.write_text(json.dumps(_with_mesh(fixture_data)))
    return _render(p)


def test_enrichment_facts_render(mesh_rendered):
    assert mesh_rendered["mesh"]["enrichmentBlocks"] >= 1
    assert mesh_rendered["mesh"]["enrichFacts"] == 2


def test_related_links_render(mesh_rendered):
    assert mesh_rendered["mesh"]["relatedBlocks"] >= 1
    assert mesh_rendered["mesh"]["relatedLinks"] >= 1


def test_entity_explorer_renders_nodes(mesh_rendered):
    assert mesh_rendered["mesh"]["entityMapBtn"] is True
    assert mesh_rendered["mesh"]["entityNodes"] == 2


def test_integrations_badge_names_active_enrichers(mesh_rendered):
    assert "Federal awards (USAspending)" in (mesh_rendered["mesh"]["integrationsBadge"] or "")


def test_mesh_absent_when_no_enrichment(rendered):
    # The base fixture has no mesh data → no enrichment/related blocks, no badge.
    assert rendered["mesh"]["enrichmentBlocks"] == 0
    assert rendered["mesh"]["relatedBlocks"] == 0
    assert rendered["mesh"]["entityMapBtn"] is False
    assert rendered["mesh"]["integrationsBadge"] is None


# ─── BTX Revenue Cockpit: brief → account-map cross-link ────────────────────

def test_account_map_link_renders_when_configured(fixture_data, tmp_path):
    data = json.loads(json.dumps(fixture_data))
    data.setdefault("site_config", {})["account_map_url"] = "map.html"
    p = tmp_path / "withmap.json"
    p.write_text(json.dumps(data))
    rendered = _render(p)
    assert rendered["mesh"]["accountMapLink"] == "map.html"


def test_account_map_link_absent_by_default(rendered):
    assert rendered["mesh"]["accountMapLink"] is None
