...
_BTX_INTAKE = (
    Path(__file__).parent.parent / "clients" / "btx" / "intake.json"
)
...
class TestIntakeSchemaConsistency:
    ...
    def test_btx_intake_profile_round_trips(self):
        intake = json.loads(_BTX_INTAKE.read_text())
...
def intake() -> dict:
    return json.loads(_BTX_INTAKE.read_text())
...
class TestScaffoldOutput:
    ...
    def test_source_briefs_carry_coverage(self, result, intake):
        ...
        assert len(briefs) == len(intake["coverage"]) + len(intake.get("paid_sources", []))
        assert any(b["name"] == "SpaceNews" for b in briefs)
...
# ─── Deterministic round-trip vs. the BTX intake ────────────────────────────

class TestRoundTrip:
    def test_branding(self, result):
        b = result["config"]["branding"]
        assert b["name"] == "BTX Aerospace & Defense Monitor"
        assert b["accent_color"] == "#1E3A8A"

    def test_editions_ids_and_categories(self, result):
        eds = result["config"]["editions"]
        assert [e["id"] for e in eds] == ["business-development", "executive-brief"]
        bd = next(e for e in eds if e["id"] == "business-development")
        assert "Contract Award" in bd["categories"]
        assert bd["analysis_instructions"].startswith("Prioritise contract awards")

    def test_never_discard(self, result):
        nd = result["config"]["scoring_rubric"]["never_discard"]
        assert "debarment" in nd and "continuing resolution" in nd
    ...
    def test_prefilter(self, result):
        ...
        assert "Lockheed Martin" in pf["include"] and "Contract Award" in pf["include"]
